...
class KafkaReaderSpout(Spout):
    ...
    def initialize(self,
                   ...
                   statsd=None):
        """Initialize the reader by connecting to Kafka."""
        self._counter = 0
        self._commit_interval = commit_interval
        # can pass in a StatsD connection
        self._statsd = statsd
        ...
        topic = client.topics['ibeng']
        self.consumer = topic.get_balanced_consumer(consumer_group='tests',
                                                    consumer_timeout_ms=100,
                                                    rebalance_max_retries=30,
                                                    auto_offset_reset=OffsetType.EARLIEST,
                                                    auto_commit_enable=False,
                                                    auto_commit_interval_ms=commit_interval * 1000,
                                                    zookeeper=zk)
    ...
    def emit_next(self):
        ...
        try:
            unpacked = json.loads(msg.value)
        except:
            log.warning("Message not JSON: %r", unpacked)
            # drop the tuple
            return
        #self._metrics.msg_read(unpacked)
        if self.is_ok(unpacked):
            data = self.get_data(unpacked)
            stream = self.get_stream(data)
            self.emit_tuple(data, stream=stream)
        else:
            log.debug('Message not okay: %s', msg)
